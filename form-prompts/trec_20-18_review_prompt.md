# TREC Form 20-18 — Contract Review Agent Prompt
...
A short table: parties, property, sales price, financing type, earnest money, option fee, option period, closing date, possession, title company, escrow agent.

### 2. Deadline Timeline
Bulleted list of every key date, computed from the Effective Date.

### 3. Addenda Checklist
Two columns: addenda checked in ¶22 vs. addenda physically attached. Mark each row ✅ match, ⚠️ mismatch, or ❌ missing.

### 4. Completeness & Execution
Confirm: Effective Date present, all required blanks filled, all required boxes checked, all required signatures and initials, broker information page complete. Note every gap.

### 5. Red Flags
List each issue with severity:
- 🔴 **Critical** — likely unenforceable, UPL exposure, or missing required addendum
- 🟡 **Caution** — internal inconsistency, missing initial, ambiguous deadline, blank that should be filled
- 🔵 **Note** — non-blocking observation

For each item: paragraph reference, exact quote of the offending text (if any), why it's a problem, and a recommended fix (e.g., "use Amendment Form 39-10," "use Third Party Financing Addendum," "recommend parties consult an attorney before adding this language").

### 6. Special Provisions (¶11) Audit
Verbatim quote of ¶11, then a line-by-line classification (OK / REDUNDANT / POTENTIAL UPL) with reasoning.

### 7. Recommended Next Steps
Plain-English action list for the agent: what to fix before submission, what to send to the attorney, what to ask the parties to clarify.
